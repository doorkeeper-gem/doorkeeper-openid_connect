...
module Doorkeeper
  module OpenidConnect
    ...
    # for each relevant client a signed Logout Token (the configured
    # `logout_token_class`) is POSTed to the client's
    ...
    module BackchannelLogout
      ...
      def deliver(resource_owner, application)
        ...
        token_class = ::Doorkeeper::OpenidConnect.configuration.logout_token_model
        token = token_class.new(resource_owner, application).as_jws_token

        response = post_logout_token(uri, token)
        Result.new(application: application, uri: uri.to_s, status: response.code.to_i)
      rescue StandardError => e
        log_delivery_failure(application, e)
        Result.new(application: application, uri: application.backchannel_logout_uri, error: e)
      end

      # §2.8 tells the OP not to treat an RP's errors as fatal, so a failed
      # delivery is only recorded — here for the operator, and in the Result
      # the caller receives.
      def log_delivery_failure(application, error)
        ::Rails.logger.warn(
      ...
      private_class_method :log_delivery_failure
      ...
      def applications_for(resource_owner)
        ...
        application_model
          ...
          .where(id: application_ids_for(resource_owner))
      end

      # The ids of every application holding a token or grant for this
      # resource owner.
      def application_ids_for(resource_owner)
      ...
      private_class_method :application_ids_for
