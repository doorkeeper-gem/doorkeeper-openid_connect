    ...
    module Helpers
      module Controller
        ...
        module MaxAge
          ...
          # How far ahead of the provider's clock an auth_time may sit and
          # still be trusted. An authentication cannot have happened in the
          # future, but the servers that write auth_time and the one that
          # checks it do not share a clock, and a user who authenticated a
          # moment ago must not be sent to reauthenticate over ordinary skew —
          # the fresh auth_time from that reauthentication would be ahead
          # again, looping forever.
          OIDC_AUTH_TIME_FUTURE_LEEWAY = 60
          ...
          # An auth_time further in the future than clock skew explains is not
          # a time the resource owner authenticated at, whatever its type — a
          # `Time`, an Integer epoch and a parsed String all reach here — so it
          # is treated like an unknown one and requires reauthentication.
          # Left unchecked, its negative age is below any `max_age` and it
          # would read as fresh indefinitely.
          def oidc_auth_time_stale?(auth_time, max_age)
            return true unless auth_time

            age = Time.zone.now - auth_time
            age > max_age || age < -OIDC_AUTH_TIME_FUTURE_LEEWAY
          end
