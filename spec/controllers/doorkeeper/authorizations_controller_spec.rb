  ...
  describe "#handle_oidc_max_age_param!" do
...

    {
      ...
      context "when auth_time_from_resource_owner returns #{type_description}" do
        ...
        it "renders the authorization form if the last login is ahead by no more than clock skew" do
          user.update! current_sign_in_at: 30.seconds.from_now
          authorize! max_age: 10

          expect_authorization_form!
        end

        it "reauthenticates the user if the last login is further in the future than clock skew" do
          user.update! current_sign_in_at: 2.minutes.from_now
          authorize! max_age: 10
    ...
    context "when auth_time_from_session is configured" do
      ...
      context "when the session round-tripped the auth_time through a serializer" do
        ...
        it "reauthenticates on a well-formed String holding a future time" do
          authorize_with_session!(
            { current_session_auth_time: "2999-09-05 03:59:27 UTC" },
            max_age: 10,
          )
