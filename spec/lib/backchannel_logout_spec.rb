...
describe Doorkeeper::OpenidConnect::BackchannelLogout do
  ...
  describe ".notify" do
    ...
    context "when logout_token_class is configured" do
      let(:custom_class) do
        Class.new(Doorkeeper::OpenidConnect::LogoutToken) do
          def claims
            super.merge(jti: "custom-jti")
          end
        end
      end

      before do
        stub_const("CustomLogoutToken", custom_class)

        # The gem seam, not `configure`: reconfiguring here would drop the
        # dummy application's issuer and signing key along with it.
        allow(Doorkeeper::OpenidConnect.configuration)
          .to receive(:logout_token_model).and_return(CustomLogoutToken)
      end

      it "delivers a token built by the configured class" do
        described_class.notify(user, applications: [application])

        claims, = logout_token_from(sent_requests.sole)
        expect(claims["jti"]).to eq "custom-jti"
      end
    end
  end
