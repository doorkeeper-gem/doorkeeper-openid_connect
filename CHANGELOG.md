...
- [#322] Fall back to Doorkeeper's own `issuer` configuration when the OpenID Connect `issuer` is not set. Doorkeeper added a top-level `issuer` option for RFC 8414 Authorization Server Metadata ([doorkeeper#1838](https://github.com/doorkeeper-gem/doorkeeper/pull/1838)); since RFC 8414's `issuer` and the OIDC `iss` claim identify the same authorization server, `resolve_issuer` now uses `Doorkeeper.config.issuer` when the OpenID Connect `issuer` is not set. The OpenID Connect `issuer` still takes precedence (backward compatible), and when neither is configured the existing `InvalidConfiguration` behavior is preserved. The fallback is guarded with `respond_to?` so it is a no-op on Doorkeeper versions that predate `config.issuer`. The install generator's initializer template now documents the fallback and precedence ([#321](https://github.com/doorkeeper-gem/doorkeeper-openid_connect/issues/321))
...
- [#334] Allow Doorkeeper 6.0: the gemspec dependency is now `>= 5.5, < 7.0` and CI pins the 6.0 series via `gemfiles/doorkeeper_6.0.gemfile`. Note for upgraders: Doorkeeper 6.0's `force_pkce` applies to confidential clients too ([doorkeeper#1842](https://github.com/doorkeeper-gem/doorkeeper/pull/1842)) and, as before, does not exempt response types that never issue an authorization code — with `force_pkce` enabled, OpenID Connect `id_token` / `id_token token` authorization requests are rejected unless they carry a `code_challenge`
- [#335] Add escape hatches via `id_token_class` and `user_info_class` for advanced customization of the ID Token and UserInfo response objects.
- [#337] Rework `id_token_class` / `user_info_class` internals — lazy-resolve and validate configured classes at first use (fixes zeitwerk on Rails 7+), unify hybrid `id_token token` token construction through `HybridIdTokenConcern`, and deprecate `Doorkeeper::OpenidConnect::IdTokenToken`. A custom `id_token_class` must now expose an `#access_token` reader. **Breaking (type change)**: the hybrid response token is now the configured `id_token_class` extended with `HybridIdTokenConcern`, not an `IdTokenToken` instance — see the [migration guide](https://github.com/doorkeeper-gem/doorkeeper-openid_connect/wiki/Migration-from-Old-Versions) for details.
- [#338] **Breaking:** Remove the deprecated `Doorkeeper::OpenidConnect::IdTokenToken` class. Since [#337] it was an empty shim over `IdToken` + `HybridIdTokenConcern` that warned on first reference, and nothing in the library references it anymore — the hybrid `id_token token` flow builds its token from the configured `id_token_class` extended with `HybridIdTokenConcern`. Custom classes subclassing `IdTokenToken` must subclass `IdToken` and include `HybridIdTokenConcern` instead
...
- [#349] Claims pipeline fixes:
  ...
  - Likewise omit `id_token` when the access token has no application, since the REQUIRED `aud` claim cannot be sourced (previously a 500 via `MissingRequiredClaim`; reachable through the refresh-token flow)
...
- [#363] Extend [#349]'s application-less `id_token` guard to the password grant, which it could not reach. A `grant_type=password` request that carries `scope=openid` while `skip_client_authentication_for_password_grant` is enabled succeeds with no client at all, so the access token it commits has no application and its ID Token cannot source the REQUIRED `aud` claim. The grant presets an ID Token before the response is rendered, and [#349]'s guard only runs when nothing was preset, so rendering the body still raised `Errors::MissingRequiredClaim` — an unhandled 500, after the access token row had already been committed. A preset ID Token is now dropped under the same precondition, making the response a normal `200` that omits `id_token`, which is what the refresh-token flow already answered for that very token. OpenID Connect Core 1.0 does not define the resource owner password credentials grant, so its token response is not required to carry an ID Token
...
- [#367] Docs: refresh the README — list the newer supported specs in the status section (RP-Initiated Logout client metadata, the RFC 9207 `iss` authorization response parameter, RFC 8414 Authorization Server Metadata), correct the development note claiming the latest Rails version is used by default (the `Gemfile` pins Rails 8.0), and point the GitLab example links at the repository's current home. No behavior change
- [#370] Docs: align the README with the wiki — add a Requirements table for the supported Ruby, Rails, and Doorkeeper versions, replace the hand-maintained list of wiki pages with a pointer to the wiki home, drop the `add_post_logout_redirect_uris` steps that the Migration and RP-Initiated Logout wiki pages already document, trim the status list back to a spec inventory, add a total-downloads badge next to the gem version badge it links alongside, and fix the table of contents link to the Configuration section and the repository-relative DiscoveryController link. The stale `rake db:migrate` command, which Rails 5 replaced with the `rails` command, is corrected everywhere the repository still names it: the README, the unreleased [#243] migration note, and the `post_logout_redirect_uris` missing-column error message. No behavior change beyond that error message's text
- [#371] Docs: describe the GitHub Actions workflows in `CONTRIBUTING.md` — the file still pointed at a Travis CI build and a `.travis.yml` that the repository does not have — and create the feature branch with `git checkout -b` rather than `-B`, which resets an existing branch of that name
- [#373] Fix `prompt=consent` under Doorkeeper's `api_only` mode: the consent step rendered the `:new` template, which ActionController::API cannot do, so the authorization request answered with an empty `200 text/plain` and no exception instead of presenting the consent step. It now answers with the pre-authorization as JSON, the same way Doorkeeper's own `AuthorizationsController` presents that step in API mode. This was the gem's only view render, so the long-standing "API mode is not properly supported" note has been dropped from the README
- Add entry here
