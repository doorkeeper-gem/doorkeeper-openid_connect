...
- [#365] Validate the initializer signature of `id_token_class` / `user_info_class` overrides at first use ([#339](https://github.com/doorkeeper-gem/doorkeeper-openid_connect/issues/339)) — a subclass whose `initialize` cannot accept the library's `(access_token, nonce)` / `(access_token)` calls now raises `InvalidConfiguration` alongside the other class-override validations, instead of an `ArgumentError` in the middle of a token or userinfo request
