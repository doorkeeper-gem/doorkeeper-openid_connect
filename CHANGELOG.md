...
- [#337] Rework the `id_token_class` / `user_info_class` internals: constantize and validate the configured classes lazily at first use instead of inside `Doorkeeper::OpenidConnect.configure` — constantizing an app-defined (reloadable) class while initializers run breaks zeitwerk on Rails 7+ — and stop memoizing the resolved class so code reloading in development never returns a stale class. The required-methods check now also accepts private implementations (`instance_methods` only reported public ones). The hybrid `id_token token` response now builds the token the same way for the default and custom classes (`id_token_model.new(...).extend(HybridIdTokenConcern)`), and `Doorkeeper::OpenidConnect::IdTokenToken` is deprecated — it is now autoloaded and warns on first reference; use `id_token_class` with `HybridIdTokenConcern` instead. A custom `id_token_class` must expose the access token via an `#access_token` reader (public or private) — `HybridIdTokenConcern` reads it instead of the `@access_token` ivar, and its presence is validated at first use along with the other required methods. **Breaking (type change)**: the token object in the hybrid `id_token token` response is no longer an instance of `Doorkeeper::OpenidConnect::IdTokenToken` — it is now the configured `id_token_class` (`IdToken` by default) extended with `HybridIdTokenConcern` — so `is_a?(Doorkeeper::OpenidConnect::IdTokenToken)` checks stop matching; check against `IdToken` (or `HybridIdTokenConcern`) instead. Duck-typed usage is unaffected: the object responds to the same methods and serializes identically
- Add entry here
