...
- [#364] Require `id_token_class` / `user_info_class` overrides to inherit from `Doorkeeper::OpenidConnect::IdToken` / `UserInfo` ([#344](https://github.com/doorkeeper-gem/doorkeeper-openid_connect/issues/344)). The previous method-presence validation was effectively a no-op for `user_info_class` — ActiveSupport defines `as_json` on `Object`, so any class passed — and a from-scratch implementation had to reproduce security-critical behavior (required-claim enforcement, the claim merge order that keeps custom claims from overriding `sub`/`aud`/`exp`, nonce and `at_hash` handling); subclasses now override only what they need. Also adds an `IdToken#select_key` override hook that returns the signing key material and algorithm together (`IdToken::SigningKey`) for per-client keys, rotation, or multi-tenant setups, and binds the hybrid flow's `at_hash` digest to the selected key's algorithm — the `alg` of the token's actual JOSE header (OIDC Core §3.2.2.10) — instead of the global `signing_algorithm`. The class-override feature ([#335]) has not shipped in a release, so no released configuration is affected
- [#365] Validate the initializer signature of `id_token_class` / `user_info_class` overrides at first use ([#339](https://github.com/doorkeeper-gem/doorkeeper-openid_connect/issues/339)). The library instantiates the ID token with `(access_token, nonce)` / `(access_token)` and the UserInfo response with `(access_token)`; a subclass whose `initialize` cannot accept those calls (wrong positional arity, or a required keyword argument) previously surfaced as an `ArgumentError` in the middle of a token or userinfo request, and is now reported as an `InvalidConfiguration` error alongside the other class-override validations
- Add entry here
