...
- [#359] CI: add an eager-load job. `bundle exec rake eager_load` boots the test application with `config.eager_load = true`, forces every `autoload` this gem registers, and fails when a file under `lib/` turns out to be loaded by nothing at all. A broken `require` fails at boot, where CI sees it; a broken `autoload` only fails at the first reference to the constant, which in a host application means a `NameError` on some request after the release went out. This restores the boot-time safety net ahead of the planned `require` → `autoload` migration of `lib/doorkeeper/openid_connect.rb`. The job runs when a pull request touches `require` / `autoload` lines or adds, removes or renames files under `lib/` or `app/`, and can be requested on any pull request with the `eager-load-check` label. Also removes four unused `rails new` stubs from `spec/dummy` (`ApplicationCable::Channel`, `ApplicationCable::Connection`, `ApplicationJob`, `ApplicationMailer`) that subclass base classes from railties the test application never loads, and so could not be eager loaded
- [#360] Load the OpenID Connect-namespaced constants with `autoload` instead of `require`, the way `lib/doorkeeper.rb` loads Doorkeeper's own. Thirteen files — the claims classes, `Errors`, `IdToken`, `UserInfo`, `ClaimsBuilder`, `HybridIdTokenConcern`, the three discovery mixins, the version constants and the dynamic client registration request — are now loaded on first reference rather than on every boot. No public constant changes name or moves. Files that only exist for a load-time side effect (`config.rb`, `engine.rb`, `helpers/controller.rb` and the five `oauth/*` files that prepend onto Doorkeeper's classes) keep their `require`, with a comment on each explaining why; a new spec asserts every one of those prepends is in place after a plain boot, so a later conversion cannot silently drop one
- [#362] Finish the `require` → `autoload` migration. The four request/response classes this gem contributes to Doorkeeper's own `OAuth` namespace (`IdTokenRequest`, `IdTokenResponse`, `IdTokenTokenRequest`, `IdTokenTokenResponse`) and the Doorkeeper 6.0 `MetadataResponse` subclass are now autoloaded, so an application that never uses the OpenID Connect implicit or hybrid flows never loads them. A normal boot now loads 23 of the gem's files instead of 35. Everything still required is required deliberately, with the reason written next to it: the two `Request` strategies, which the `GrantFlow` registration names as it runs; `orm/active_record.rb`, which prepends onto Doorkeeper's access grant mixin and picks its Doorkeeper 5.5 fallback at load time; and `rails/routes.rb`, which the engine initializer loads on every boot anyway
- Add entry here
